...
async function createNotification(employeeId, title, message, type) {
  try {
    const cleanEmpId = employeeId.toString().trim().replaceAll(RegExp('^0+'), '');
    const query = `
...
router.post('/leaves/approve', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (nextStatus === 'APPROVED') {
      await createNotification(applicantId, 'Leave Request Approved', 'Your leave request has been fully approved.', 'Leave');
    } else if (nextStatus === 'SENT_L2') {
      await createNotification(applicantId, 'Leave Request L1 Approved', 'Your leave request has been approved at L1 and is pending L2 approval.', 'Leave');
    }
...
router.post('/leaves/reject', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (l1 == managerId || l2 == managerId) {
      ...
      await createNotification(applicantId, 'Leave Request Rejected', `Your leave request has been rejected. Remarks: ${remarks || 'None'}`, 'Leave');
...
router.post('/api/tours/approve', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (nextStatus === '2') {
      await createNotification(tour.personnel_number, 'Tour Request Approved', `Your tour request to ${tour.trip_destination || 'destination'} has been fully approved.`, 'Tour');
    } else if (nextStatus === '11') {
      await createNotification(tour.personnel_number, 'Tour Request L1 Approved', `Your tour request to ${tour.trip_destination || 'destination'} has been approved at L1 and is pending L2 approval.`, 'Tour');
    }
...
router.post('/tours/reject', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (tour.reporting_officer == managerId || tour.reporting_officer_1 == managerId) {
      ...
      await createNotification(tour.personnel_number, 'Tour Request Rejected', `Your tour request to ${tour.trip_destination || 'destination'} has been rejected. Remarks: ${remarks || 'None'}`, 'Tour');
...
router.get('/notifications', authenticateToken, async (req, res) => {
  const employeeId = req.user.employee_number;
  try {
    const [rows] = await pool.query('SELECT * FROM notifications WHERE employee_id = ? ORDER BY created_at DESC', [employeeId]);
...
router.post('/notifications/read', authenticateToken, async (req, res) => {
  const employeeId = req.user.employee_number;
  try {
    await pool.query('UPDATE notifications SET is_read = TRUE WHERE employee_id = ?', [employeeId]);
...
router.post('/leave-encashment', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [quotaRows] = await pool.query(
      `SELECT * FROM leave_quota WHERE personnel_number = ? AND absence_quota_type = '1000' ORDER BY deduction_to DESC LIMIT 1`,
      [cleanEmpId]
    );
