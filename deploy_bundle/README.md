...
- **URL**: `POST /api/login`
- **Description**: Authenticates an employee using their Personnel/Employee Number and Date of Birth (YYYY-MM-DD format). Returns a signed JWT token valid for 24 hours alongside their full HR profile.
- **Request Body (JSON)**:
  ```json
  {
    "employee_number": 141,
...
- **Example Response (Failure)**:
  ```json
  {
    "error": "Invalid Employee Number or Date of Birth"
  }
  ```
