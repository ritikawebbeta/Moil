...
function mapEmployeeRow(row) {
  ...
  return {
    ...
    permanentAddress: (row.permanent_address || row.perm_address || row.address || '').toString().trim() || 'Flat No. 102, Plot No. 37/38, Vandana Mension, Surana Layout, Anant Nagar Nagpur 440013',
    temporaryAddress: (row.temporary_address || row.temp_address || row.present_address || '').toString().trim() || 'Flat No. 102, Plot No. 37/38, Vandana Mension, Surana Layout, Anant Nagar Nagpur 440013',
    emergencyAddress: (row.emergency_address || row.current_address || row.emerg_address || '').toString().trim() || 'Moil Bhawan Katol Road Nagpur 440013',
    currentAddress: (row.current_address || row.emergency_address || row.emerg_address || '').toString().trim() || 'Moil Bhawan Katol Road Nagpur 440013',
 ...
 * @desc    Send SMS notification using MOIL DLT templates
 */
router.post('/send-sms', async (req, res) => {
  const {
    template_id, templateId,
    mobile, mobileNumber,
    script, dlt_template_id
  } = req.body;

  const targetPhone = mobile || mobileNumber || smsService.DEFAULT_MOBILE;
  const tId = dlt_template_id || template_id || templateId || '1107163177301329708';
